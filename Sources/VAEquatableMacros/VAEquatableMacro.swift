...
public struct VAEquatableMacro: ExtensionMacro {
    ...
    public static func expansion(
        ...
        let properties = try declaration.getEquatableProperties(withComputed: withComputed, isPublic: declaration.modifiers.isPublic).equatableNames
        let isAlreadyEquatable = declaration.inheritanceClause?.inheritedTypes.contains(where: { $0.type.trimmedDescription == .equatable }) ?? false

        return [
            ExtensionDeclSyntax(
                extendedType: type,
                inheritanceClause: isAlreadyEquatable ? nil : InheritanceClauseSyntax(
                    inheritedTypes: [InheritedTypeSyntax(type: IdentifierTypeSyntax(name: TokenSyntax.identifier(.equatable)))]
                )
            ) {
                FunctionDeclSyntax(
